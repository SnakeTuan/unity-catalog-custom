...
// calls stsClient.assumeRole() with:
// - roleArn - the IAM role to assume (from CredentialDAO or per-bucket config)
// - policy - scoped-down IAM policy generated by AwsPolicyGenerator (restricts to specific S3 paths with SELECT/UPDATE actions)
// - externalId - anti-confused-deputy token (only for external location credentials)
